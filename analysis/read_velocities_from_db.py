...
def main():
    ...
    parser.add_argument(
        "--batch", type=int, help="Which batch of bees to download the data for. (0-199)"
    )
    ...
    def save_bee_velocities(cursor) -> None:
        """
        Get velocities of all alive bees in the given time range and save them to a cache_directory.
        
        Parameters:
            cursor: Database cursor to use for fetching data.
        """
        ...
        for bee_id in current_batch:
            ...
            if not os.path.exists(filepath):
                start = time.time()
                df = db.trajectory.get_bee_velocities(
                    bee_id,
                    dt_from,
                    dt_to,
                    additional_columns=[
                    ...
                    cursor=cursor,
                    cursor_is_prepared=cursor_is_prepared,
                )
    ...
    if year == 2019:
        ...
        db.base.beesbook_season_config[
            "bb_detections"
